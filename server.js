...
function pickVideoFilter({ preset, resolution }) {
  ...
  if (preset === "clarity") {
    return `${scale},eq=contrast=1.05:brightness=0.01:saturation=1.04,unsharp=3:3:0.4:3:3:0.0`;
  }
  if (preset === "social") {
    return `${scale},eq=contrast=1.07:brightness=0.02:saturation=1.06`;
  }
  if (preset === "cinematic") {
    return `${scale},hqdn3d=0.6:0.6:2:2,eq=contrast=1.03:brightness=0.01:saturation=1.02`;
  }
...
/**
 * IMPORTANT:
 * ElevenLabs audio isolation expects multipart field name: "audio"
 * We first extract audio from the trimmed video, then upload that audio file.
 */
async function runElevenLabsIsolation(videoInputPath, originalName, elevenKey) {
  const extractedAudioPath = uniqueTemp("source-audio.mp3");
  const isolatedAudioPath = uniqueTemp("isolated-audio.mp3");

  // Step 1: extract audio from video
  await extractAudioToMp3(videoInputPath, extractedAudioPath);

  // Step 2: send extracted audio to ElevenLabs
  const form = new FormData();
  form.append("audio", fs.createReadStream(extractedAudioPath), {
    filename: (originalName || "input.mp4").replace(/\.[^.]+$/, ".mp3"),
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new Error(
        `ElevenLabs failed (${response.status}): ${decodeAxiosErrorData(
          response.data
        )}`
      );
    }

    await fsp.writeFile(isolatedAudioPath, Buffer.from(response.data));
    cleanup([extractedAudioPath]);
    return isolatedAudioPath;
  } catch (err) {
    cleanup([extractedAudioPath, isolatedAudioPath]);
    if (err.response) {
      throw new Error(
        `ElevenLabs failed (${err.response.status}): ${decodeAxiosErrorData(
          err.response.data
        )}`
      );
    }
...
async function runFalUpscale(inputPath, falKey, modelId, scaleFactor) {
...
app.post("/api/enhance-fast", upload.single("media"), async (req, res) => {
  ...
  try {
    ...
    await runFfmpeg({
      ...
      trimStart,
      clipDuration,
      videoFilter: pickVideoFilter({
        preset: req.body.preset,
        resolution: req.body.resolution,
      }),
      audioFilter: pickAudioFilter({
        audioPreset: req.body.audioPreset,
        audioGain: req.body.audioGain,
      }),
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="fast-enhanced.mp4"'
    );
...
app.post("/api/enhance-ai", upload.single("media"), async (req, res) => {
  ...
  try {
    ...
    const modelId = (
      req.body.falModelId || "clarityai/crystal-video-upscaler"
    ).trim();
    ...
    if (!elevenKey || !falKey) {
      return res.status(400).json({
        error:
          "Both ElevenLabs and fal API keys are required for AI Enhance.",
      });
    }

    const { trimStart, clipDuration } = await buildTrimInfo(req, inputPath);

    // Trim first so AI only processes the selected clip
    trimmedInputPath = uniqueTemp("trimmed-input.mp4");
    await runFfmpeg({
      inputPath,
    ...
    // ElevenLabs gets extracted audio
    isolatedAudioPath = await runElevenLabsIsolation(
      inputPath,
      req.file.originalname,
      elevenKey
    );

    // fal gets trimmed video
    aiVideoPath = await runFalUpscale(
      inputPath,
      falKey,
      modelId,
      scaleFactor
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="ai-enhanced.mp4"'
    );
    ...
    cleanup([inputPath, trimmedInputPath, aiVideoPath, isolatedAudioPath, outputPath]);
    res.status(error.statusCode || 500).json({
      error: error.message || "AI enhancement failed.",
      details:
        "Check that your ElevenLabs key, fal key, and selected fal model are valid and enabled on your account.",
      stderr: error.ffmpegStderr || "",
    });
  }
});
